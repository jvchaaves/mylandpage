...
export const projects: Project[] = [
  ...
  },
  {
    ...
    howItWorks:
      "O paciente envia uma mensagem no WhatsApp e o gateway Node.js (whatsapp-web.js) faz o pré-processamento: detecta o tipo de mídia, baixa o arquivo e encaminha ao brain via HTTP. Se for áudio OGG, o brain baixa e transcreve com Whisper-1 antes de passar ao agente. Se for imagem, constrói uma mensagem multimodal com a URL da mídia para análise visual pelo GPT-4o.\n\nO HipocratesAgent instancia o agente LangChain com as 12 ferramentas e inicia o loop ReAct: analisa a intenção do paciente, recupera o histórico de chat e o perfil médico do Supabase (alergias, condições crônicas, medicamentos), e decide qual ferramenta acionar. Para dúvidas clínicas, chama research_medical_topic_tool que executa busca vetorial na biblioteca médica interna e busca DuckDuckGo restrita a domínios aprovados, retornando evidências com fontes citadas. Para sintomas agudos, aciona search_triage_protocol_tool que busca protocolos de triagem na base vetorial e retorna perguntas obrigatórias de anamnese e red flags.\n\nQuando a pré-consulta está completa, generate_clinical_summary_tool salva o resumo clínico estruturado na tabela consultations do Supabase com status 'ready_for_doctor', disparando a notificação para o médico. O scheduler background verifica lembretes e consultas próximas a cada ciclo, enviando notificações proativas de preparo de exames e retornos.",
    architecture: [
      "Gateway Node.js + whatsapp-web.js — recepção e envio de mensagens WhatsApp",
      "Brain Python FastAPI — agente LangChain com 12 ferramentas clínicas",
      "RAG médico — pgvector (Supabase) + busca web restrita (Cochrane, PubMed, Fiocruz)",
      "Supabase PostgreSQL — pacientes, consultas, lembretes, prontuários, histórico de chat",
      "Scheduler background — lembretes, follow-ups e alertas pós-consulta",
      "Docker Compose — 2 serviços (gateway :3002 + brain :8002)",
    ],
    tech: [
      { name: "Python", category: "Backend" },
      { name: "FastAPI", category: "Backend" },
    ...
    features: [
      "Agente ReAct com 12 ferramentas LangChain especializadas em saúde",
      "RAG médico: base vetorial interna + web restrita (Cochrane, PubMed, NEJM, Fiocruz)",
      "Triagem com protocolo de red flags por sintoma agudo",
      "Multimodal: texto, áudio OGG (Whisper), imagens de exames (GPT-4o visão)",
      "Resposta em áudio via OpenAI TTS (voz nova)",
      "Resumo clínico automático entregue ao médico ao fim da pré-consulta",
      "Perfil médico persistido: alergias, condições crônicas, medicamentos",
      "Scheduler de lembretes, follow-ups e alertas de retorno",
    ],
  ...
  },
  {
    ...
    howItWorks:
      "O médico acessa o dashboard e visualiza a fila de pré-consultas processadas pelo agente hipocrites.AI, com cards mostrando queixa principal, urgência estimada, perfil médico do paciente (alergias, condições, medicamentos) e horário agendado. A fila atualiza automaticamente conforme novos pacientes concluem a triagem no WhatsApp.\n\nDurante a consulta, o módulo de transcrição abre um WebSocket com o Deepgram SDK que converte a fala em texto em tempo real. O transcript batcher acumula os segmentos e, ao final da sessão, envia o texto completo ao microsserviço clinical_llm que gera um resumo clínico estruturado — queixa, anamnese, hipóteses diagnósticas e conduta — com referências às questões similares encontradas via RAG no pgvector.\n\nPara importação de documentos, o médico ou secretária faz upload de PDFs de laudos e exames. O clinical_import detecta automaticamente o tipo de documento, extrai texto via pdf_text ou OCR (pytesseract sobre imagens geradas pelo pdf2image), e o structured_parser organiza os dados extraídos em campos estruturados (data, valor, unidade, referência) para inserção no prontuário do paciente.",
    architecture: [
      "Frontend React/Vite (TypeScript + Tailwind) — dashboard médico, fila e transcrição",
      "Backend Node.js/TypeScript (Express) — módulos: appointments, consultations, dashboard, doctors, patients, storage",
      "Microsserviço Python clinical_llm (FastAPI) — embeddings, RAG (pgvector) e resumos LLM",
      "Microsserviço Python clinical_import — OCR (PyMuPDF + pytesseract) e parsing estruturado",
      "Deepgram SDK — transcrição ao vivo de consultas",
      "Supabase PostgreSQL — prontuários, agendamentos, consultas, storage",
      "@hipo/contracts — Zod schemas compartilhados entre frontend e backend",
    ],
    tech: [
    ...
    features: [
      "Dashboard médico com fila de pré-consultas processadas por IA",
      "Transcrição ao vivo de consultas via Deepgram WebSocket",
      "Resumos clínicos automáticos com RAG (pgvector) pós-consulta",
      "Importação de laudos e exames com OCR (PyMuPDF + pytesseract)",
      "Parsing estruturado de documentos clínicos (laudos, receitas, exames)",
      "API documentada com OpenAPI/Swagger via Zod schemas compartilhados",
      "Módulos independentes: appointments, consultations, patients, storage",
    ],
    github: "https://github.com/henriquefsousa/HipocratesAi-Front",
