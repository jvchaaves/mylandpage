 ...
 * É o que sinaliza "aqui dentro tem um sistema" sem precisar escrever.
 */
function PreferenciasIcone() {
  return (
    <svg viewBox="0 0 64 64" className="h-10 w-10" aria-hidden>
      ...
      {/* sulco que separa o interruptor do resto da placa */}
      <path d="M27 6v52" stroke="#b4b9be" strokeWidth="1.2" />
      <path d="M28 6v52" stroke="#fdfdfd" strokeWidth="1" />

      {/* interruptor de parede, alavanca para cima */}
...
export default function MacCorner({ label }: { label: string }) {
  return (
    <Link
      href="/os"
      aria-label={label}
      className="group fixed bottom-5 right-5 z-40 hidden items-center gap-2 rounded-2xl bg-white/95 p-2 shadow-[0_6px_22px_rgba(0,0,0,0.35)] ring-1 ring-black/10 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_10px_28px_rgba(0,0,0,0.45)] sm:flex print:hidden"
    >
      <PreferenciasIcone />

      {/* O rótulo nasce com largura zero e abre no hover. */}
      <span className="max-w-0 overflow-hidden whitespace-nowrap text-sm text-neutral-700 transition-all duration-300 ease-smooth group-hover:max-w-[180px] group-hover:pr-2">
        {label}
      </span>
    </Link>
  );
